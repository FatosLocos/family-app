...
class LocalProbeConsumer(AsyncJsonWebsocketConsumer):
    ...
    async def receive_json(self, content, **kwargs):
        ...
        try:
            if event_type == "heartbeat":
                await self._seen(content.get("version", ""), content.get("adapters"))
                ...
                await self._command_result(bool(content.get("succeeded")), content.get("error", ""))
    ...
    @database_sync_to_async
    def _seen(self, version="", adapters=None):
        mark_probe_seen(self.probe, version, adapters)
    ...
    @database_sync_to_async
    def _command_result(self, succeeded, error):
        record_probe_command_result(self.probe, succeeded, error)
